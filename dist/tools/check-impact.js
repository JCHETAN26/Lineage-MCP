...
export async function checkImpact(input, graph) {
    ...
    const rawResults = await Promise.all(candidates.map(async (dep) => {
        let content = "";
        let snippet;
        try {
        ...
        if (column) {
            if (!columnMatchForDependency(content, dep, table, column))
                return null;
        }
        // Boost confidence if explicit column dep; keep dep confidence otherwise
        const isExplicitColDep = columnDeps.some((c) => c.filePath === dep.filePath && c.line === dep.line);
        const confidence = isExplicitColDep
            ? "high"
            : column
                ? "medium"
                : dep.confidence;
    ...
    const affectedFiles = rawResults.filter((f) => f !== null);
    const assetLabel = column ? `${table}.${column}` : table;
    const summary = affectedFiles.length === 0
...
function columnMatchForDependency(content, dep, table, column) {
    ...
    if (!statement)
        return false;
    if (!colRe.test(statement))
        return false;
    const tableAliases = extractSqlAliases(statement, table);
    if (tableAliases.length === 0) {
        return colRe.test(statement);
    }
    const aliasQualified = tableAliases.some((alias) => new RegExp(`\\b${escapeRegExp(alias)}\\s*\\.\\s*${escapeRegExp(column)}\\b`, "i").test(statement));
    if (aliasQualified)
        return true;
...
function findSqlStatementForLine(content, line) {
    const lines = content.split("\n");
...
function extractSqlAliases(statement, table) {
    const aliases = new Set();
    const re = new RegExp(`\\b(?:FROM|JOIN|UPDATE|INTO)\\s+${escapeRegExp(table)}\\b(?:\\s+(?:AS\\s+)?(\\w+))?`, "gi");
...
function buildFix(changeType, name, newName, pattern) {
...
//# sourceMappingURL=check-impact.js.map