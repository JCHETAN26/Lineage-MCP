  ...
  if (command === "lineage") {
    ...
    if (result.consumers.length > 0) {
      ...
      for (const consumer of result.consumers) {
        console.log(
          `- ${consumer.filePath}:${consumer.line} | ${consumer.pattern} | ${consumer.confidence} | ${capitalize(consumer.evidenceType)} | ${consumer.provenance}`
        );
      }
    }
  ...
  if (command === "impact") {
    ...
    if (result.affectedFiles.length > 0) {
      ...
      for (const file of result.affectedFiles) {
        console.log(
          `- ${file.filePath}:${file.line} | ${file.confidence} | ${capitalize(file.evidenceType)} | ${file.provenance ?? "unknown"}`
        );
        if (file.suggestedFix) console.log(`  fix: ${file.suggestedFix}`);
        if (file.snippet) console.log(indentBlock(file.snippet, "  "));
      }
    }
    return;
  }
...
function isChangeType(value: string): value is "rename" | "delete" | "type_change" | "add" {
