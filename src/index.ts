...
async function getGraph(rootDir: string, forceRescan = false): Promise<LineageGraph> {
  if (!forceRescan) {
  ...
  tools: [
    {
      name: "check_impact",
      description:
        "Analyze the blast radius of a schema change. Returns all files and line numbers that will break, with confidence levels and suggested fixes.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      name: "list_lineage",
      description:
        "Show the full dependency chain for a table or column: which files consume it, which upstream tables feed it, visual lineage tree.",
      inputSchema: {
        type: "object",
        properties: {
          table: { type: "string" },
    ...
    },
    {
      name: "scan",
      description:
        "Trigger a fresh rescan of the codebase, bypassing the cache. Use this after schema changes.",
      inputSchema: {
        type: "object",
        properties: {
          rootDir: { type: "string", default: "." },
    ...
    },
    {
      ...
      description:
        "Apply an automated fix to a file by replacing a code snippet. Creates backups before modifying. Supports dry-run mode.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Audit the lineage graph for PII exposure. Scans table and column names for sensitive data patterns and tracks downstream flows.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Synchronize discovered SQL columns with dbt YAML metadata. Identifies missing or new columns in dbt model definitions.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Generate a comprehensive health report for the lineage graph with metrics, recommendations, and visual dependency diagram.",
      inputSchema: {
        type: "object",
        properties: {
          rootDir: { type: "string", default: ".", description: "Root directory to scan" },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (name === "scan") {
      const rootDir = (a.rootDir as string) ?? ".";
...
function formatImpactReport(report: ReturnType<typeof Object.create>): string {
...
function formatLineageResult(result: ReturnType<typeof Object.create>): string {
  ...
  if (consumers.length > 0) {
    ...
    for (const c of consumers) {
      lines.push(
        `- \`${c.filePath}:${c.line}\` — ${c.pattern} [${c.confidence}] (${c.provenance}; ${capitalizeEvidence(c.evidenceType)})`
      );
    }
  }
...
function formatWarningsBlock(warnings: string[]): string {
  if (warnings.length === 0) return "";
  return `\nWarnings:\n${warnings.map((warning) => `  - ${warning}`).join("\n")}`;
}

function formatPIIComplianceReport(report: any): string {
  const lines: string[] = [
  ...
  if (report.findings.length > 0) {
    ...
    for (const finding of report.findings) {
      lines.push(
        ``,
        `**${finding.table}.${finding.column}** [${finding.riskLevel.toUpperCase()}]`,
        `Reason: ${finding.reason}`
      );
      if (finding.flows.length > 0) {
        lines.push(`Flows to:`, ...finding.flows.map((f: any) => `  - ${f.toFile}:${f.toLine}`));
      }
    }
  }
...
function capitalizeEvidence(value?: string): string {
  if (!value) return "Unknown";
  return value[0].toUpperCase() + value.slice(1);
}

function formatHealthReportSummary(report: any): string {
  const healthEmoji =
