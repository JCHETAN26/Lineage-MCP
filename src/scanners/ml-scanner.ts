...
export function scanMlFile(content: string, filePath: string): DependencyNode[] {
  ...
  for (const { re, confidence, label, provenance } of PATTERNS) {
    ...
    for (const match of content.matchAll(re)) {
      ...
      for (const name of names) {
        if (!name || name.length < 2 || SHORT_IGNORE.has(name)) continue;
        deps.push({
          type: "dependency",
          filePath,
          line,
...
function extractNames(captured: string, label: string): string[] {
...
function dedup(deps: DependencyNode[]): DependencyNode[] {
