...
> **Project status as of v1.0.0:** Phases 1–5 below are complete plus an "Agentic Data Janitor" layer (apply_remediation, audit_pii_compliance, sync_dbt_metadata, generate_health_report). 10 MCP tools registered, 136 tests pass on macOS + Linux, scanner is regex-based (Tree-sitter not integrated — kept as a post-1.0 option). Distributed as `@cjitendr/lineage-mcp`.
...
- Configure `package.json` with a `"bin"` field to support `npx @cjitendr/lineage-mcp`.
